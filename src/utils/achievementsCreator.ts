...
import TitleIcon from '@/assets/images/icons/gems1.png'; // Placeholder for titles
import PortraitIcon2 from '@/assets/images/portraits/2.png'; 
import PortraitIcon3 from '@/assets/images/portraits/3.png';
import PortraitIcon4 from '@/assets/images/portraits/4.png';
import PortraitIcon5 from '@/assets/images/portraits/5.png';
import PortraitIcon6 from '@/assets/images/portraits/normalMax.png';
import PortraitIcon7 from '@/assets/images/portraits/specialMax.png';
import PortraitIcon8 from '@/assets/images/portraits/uniqueNormal.png';
import PortraitIcon8 from '@/assets/images/portraits/love.png';
...
export const createAchievements = (): Achievement[] => {
  return [
    ...
    { id: 'clicks-5', name: 'Mining Legend', description: 'Tap 1,000,000 times', unlocked: false, checkCondition: (state: GameState) => state.totalClicks >= 1000000, rewards: [{ type: 'title', value: 'quantum_miner', image: TitleIcon }] }, // Rare
    ...
    { id: 'income-3', name: 'Mining Corporation', description: 'Earn 1,000,000,000 coins', unlocked: false, checkCondition: (state: GameState) => state.totalEarned >= 1000000000, rewards: [{ type: 'title', value: 'cosmic_explorer', image: TitleIcon }] }, // Uncommon
    { id: 'income-4', name: 'Galactic Enterprise', description: 'Earn 1,000,000,000,000 coins', unlocked: false, checkCondition: (state: GameState) => state.totalEarned >= 1000000000000, rewards: [{ type: 'title', value: 'galactic_pioneer', image: TitleIcon }] }, // Rare
    ...
    { id: 'upgrades-3', name: 'Elemental Mastery', description: 'Purchase all element upgrades', unlocked: false, checkCondition: (state: GameState) => state.upgrades.filter(u => u.category === 'element' && u.level > 0).length === state.upgrades.filter(u => u.category === 'element').length && state.upgrades.filter(u => u.category === 'element').length > 0, rewards: [{ type: 'title', value: 'asteroid_hunter', image: TitleIcon }] }, // Uncommon
    ...
    { id: 'level-3', name: 'Enhancement Master', description: 'Get any upgrade to level 500', unlocked: false, checkCondition: (state: GameState) => state.totalClicks >= 500, rewards: [{ type: 'gems', value: 20, image: GemIcon }] }, // Fixed condition typo
    ...
    { id: 'level-4', name: 'Upgrade Titan', description: 'Get 10 upgrades to level 500', unlocked: false, checkCondition: (state: GameState) => state.upgrades.filter(u => u.level >= 500).length >= 10, rewards: [{ type: 'title', value: 'stellar_commander', image: TitleIcon }] }, // Epic
    ...
    { id: 'prestige-4', name: 'Master of Reincarnation', description: 'Prestige 25 times', unlocked: false, checkCondition: (state: GameState) => state.prestigeCount >= 25, rewards: [{ type: 'title', value: 'nebula_master', image: TitleIcon }] }, // Epic
    ...
    { id: 'passive-4', name: 'Passive Emperor', description: 'Reach 1 trillion coins per second (1e12)', unlocked: false, checkCondition: (state: GameState) => state.coinsPerSecond >= 1e12, rewards: [{ type: 'title', value: 'essence_collector', image: TitleIcon }] }, // Rare
    ...
    { id: 'collection-2', name: 'Rare Collector', description: 'Own all artifacts', unlocked: false, checkCondition: (state: GameState) => state.ownedArtifacts.length >= state.artifacts.length, rewards: [{ type: 'title', value: 'space_rookie', image: TitleIcon }] }, // Common
    ...
    { id: 'abilities-2', name: 'Tech Visionary', description: 'Unlock all abilities', unlocked: false, checkCondition: (state: GameState) => state.abilities.filter(a => a.unlocked).length === state.abilities.length && state.abilities.length > 0, rewards: [{ type: 'title', value: 'void_emperor', image: TitleIcon }] }, // Legendary
    ...
    { id: 'essence-2', name: 'Essence Lord', description: 'Collect 1,000,000 essence', unlocked: false, checkCondition: (state: GameState) => state.essence >= 1000000, rewards: [{ type: 'title', value: 'cosmic_deity', image: TitleIcon }] }, // Legendary
    ...
    { id: 'skillpoints-1', name: 'Skill Accumulator', description: 'Earn 100 skill points', unlocked: false, check withCondition: (state: GameState) => state.skillPoints >= 100, rewards: [{ type: 'gems', value: 15, image: GemIcon }] },
    ...
    { id: 'gems-2', name: 'Gem Magnate', description: 'Collect 10,000 gems', unlocked: false, checkCondition: (state: GameState) => state.gems >= 10000, rewards: [{ type: 'portrait', value: 'love', image: PortraitIcon8 }] }, // Stellar Supporter: love.png
    { id: 'completion-1', name: 'Cosmic Completionist', description: 'Unlock 75% of all achievements', unlocked: false, checkCondition: (state: GameState) => state.achievements.filter(a => a.unlocked).length / state.achievements.length >= 0.75, rewards: [{ type: 'title', value: 'celestial_sovereign', image: TitleIcon }] }, // Legendary
  ];
};
