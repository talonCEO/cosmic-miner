...
export const createAchievements = (): Achievement[] => {
  return [
    ...
    { id: 'level-3', name: 'Enhancement Master', description: 'Get any upgrade to level 500', unlocked: false, checkCondition: (state: GameState) => state.upgrades.some(u => u.level >= 500), rewards: [{ type: 'gems', value: 20, image: GemIcon }] }, // Fixed condition
    ...
    { id: 'skillpoints-1', name: 'Skill Accumulator', description: 'Earn 100 skill points', unlocked: false, checkCondition: (state: GameState) => state.skillPoints >= 100, rewards: [{ type: 'gems', value: 15, image: GemIcon }] }, // Fixed typo
