...
export const calculateTapValue = (state: GameState): number => {
  ...
  const permaTapBoost = state.activeBoosts
    ...
    .reduce((sum, boost) => sum + (boost.quantity || 0) * INVENTORY_ITEMS.PERMA_TAP.effect!.value, 0);

  return Math.floor(((baseClickValue + coinsPerSecondBonus) * 
         clickMultiplier * 
         tapBoostMultiplier * 
...
export const calculateTotalCoinsPerSecond = (state: GameState): number => {
  ...
  const permaPassiveBoost = state.activeBoosts
    ...
    .reduce((sum, boost) => sum + (boost.quantity || 0) * INVENTORY_ITEMS.PERMA_PASSIVE.effect!.value, 0);

  return Math.floor((state.coinsPerSecond * 
