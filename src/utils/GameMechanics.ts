...
export const calculateTapValue = (state: GameState): number => {
  ...
  const tapBoostMultiplier = tapPowerUpgrade && 
    typeof tapPowerUpgrade.level === 'number' && 
    typeof tapPowerUpgrade.coinsPerClickBonus === 'number' 
    ? 1 + (tapPowerUpgrade.level * tapPowerUpgrade.coinsPerClickBonus) 
    : 1;

  const clickMultiplier = calculateClickMultiplier(state.ownedArtifacts);
  const baseClickValue = state.coinsPerClick || 1; // Default to 1 if undefined
  const coinsPerSecondBonus = (state.coinsPerSecond || 0) * 0.05;
  const abilityTapMultiplier = calculateAbilityTapMultiplier(state.abilities);

  let finalValue = (baseClickValue + coinsPerSecondBonus) * 
                   clickMultiplier * 
                   tapBoostMultiplier * 
                   abilityTapMultiplier;

  if (state.activeBoosts.some(b => b.id === 'boost-tap-boost' && b.remainingTime > 0)) {
    finalValue *= INVENTORY_ITEMS.TAP_BOOST.effect!.value;
  }

  return finalValue;
};

export const calculatePassiveIncome = (state: GameState, tickInterval: number = 1000): number => {
...
export const calculateTotalCoinsPerSecond = (state: GameState): number => {
  let baseCPS = state.coinsPerSecond || 0;
  ...
  return baseCPS * passiveIncomeMultiplier * artifactProductionMultiplier * managerBoostMultiplier * coinMultiplier;
};

export const calculateCoinMultiplier = (state: GameState): number => {
  let multiplier = 1;
  if (state.activeBoosts.some(b => b.id === 'boost-double-coins' && b.remainingTime > 0)) {
    ...
    if (state.ownedManagers.includes(manager.id) && manager.boosts) {
      ...
      if (manager.perks) {
        ...
        if (boostPerks.length > 0) {
          managerBoostValue = boostPerks.sort((a, b) => b.effect.value - a.effect.value)[0].effect.value;
        }
      }
...
export const calculateAutoTapIncome = (state: GameState, tickInterval: number = 1000): number => {
  ...
  const clickMultiplier = calculateClickMultiplier(state.ownedArtifacts);
  const baseClickValue = (state.coinsPerClick || 1) * 0.35;
  const coinsPerSecondBonus = (state.coinsPerSecond || 0) * 0.05;

  let autoTapAmount = (baseClickValue + coinsPerSecondBonus) * state.incomeMultiplier * clickMultiplier * 0.4 * tapBoostMultiplier * (tickInterval / 1000);
  if (state.activeBoosts.some(b => b.id === 'boost-auto-tap' && b.remainingTime > 0)) {
    autoTapAmount *= INVENTORY_ITEMS.AUTO_TAP.effect!.value;
  }
  return autoTapAmount;
};
...
export const calculateUpgradeCost = (state: GameState, upgradeId: string, quantity: number = 1): number => {
  ...
  let totalCost = calculateBulkPurchaseCost(upgrade.baseCost, upgrade.level, quantity, 1.15) * costReduction;
  if (state.activeBoosts.some(b => b.id === 'boost-cheap-upgrades' && b.remainingTime > 0)) {
...
export const calculateMaxAffordableQuantity = (coins: number, baseCost: number, currentLevel: number, growthRate: number = 1.15): number => {
  ...
  return rightSide <= 0 ? 1000 : Math.floor(Math.log(rightSide) / Math.log(growthRate));
};
...
export const calculateEssenceReward = (totalCoins: number, state: GameState): number => {
  ...
  if (state.activeBoosts.some(b => b.id === 'boost-essence-boost' && b.remainingTime > 0)) {
    multiplier *= INVENTORY_ITEMS.ESSENCE_BOOST.effect!.value;
  }
