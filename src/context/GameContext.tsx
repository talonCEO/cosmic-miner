...
export interface GameState {
  ...
  lastAutoBuy: number; // Added for auto-buy cooldown
}
  ...
  switch (action.type) {
    ...
    case 'TICK': {
      try {
        ...
        // Update boost timers and filter expired boosts
        const newActiveBoosts = newState.activeBoosts.map(boost => {
          if (boost.remainingTime !== undefined && boost.duration) {
            return { ...boost, remainingTime: Math.max(0, boost.remainingTime - 1) };
          }
          return boost;
        }).filter(boost => !boost.duration || boost.remainingTime > 0);

        // Sync state.boosts with activeBoosts
        const newBoosts = { ...state.boosts };
        Object.keys(newBoosts).forEach(boostId => {
          const activeBoost = newActiveBoosts.find(b => b.id === boostId);
          if (activeBoost) {
            newBoosts[boostId].active = true;
            newBoosts[boostId].remainingTime = activeBoost.remainingTime;
          } else {
            newBoosts[boostId].active = false;
            newBoosts[boostId].remainingTime = 0;
          }
        });

        newState = { ...newState, boosts: newBoosts, activeBoosts: newActiveBoosts };

        // Passive income (scaled for 1-second ticks)
        if (state.coinsPerSecond > 0) {
          const passiveAmount = GameMechanics.calculatePassiveIncome(state, 1000);
          newState = {
            ...newState,
        ...
        // Auto-tap (scaled for 1-second ticks)
        if (newState.autoTap) {
          const autoTapAmount = GameMechanics.calculateAutoTapIncome(state, 1000);
          newState = {
            ...newState,
        ...
        // Auto-buy with 5-second cooldown
        if (newState.autoBuy && (now - state.lastAutoBuy >= 5)) {
          const costReduction = GameMechanics.calculateCostReduction(state);
    ...
    case 'BUY_MANAGER': {
      ...
      return { ...state, essence: state.essence - manager.cost, ownedManagers: [...state.ownedManagers, action.managerId] };
    }
    case 'BUY_ARTIFACT': {
      ...
      return { ...state, essence: state.essence - artifact.cost, ownedArtifacts: [...state.ownedArtifacts, action.artifactId] };
    }
    ...
    case 'USE_ITEM': {
      ...
      if (trackedBoostIds.includes(item.id) && item.effect) {
        ...
        return { ...state, inventory: updatedInventory, activeBoosts: newActiveBoosts, boosts: newBoosts };
      }
    ...
    case 'ACTIVATE_BOOST': {
      ...
      if (existingBoostIndex >= 0) {
        ...
        newActiveBoosts[existingBoostIndex] = {
          ...
          remainingTime: (existingBoost.remainingTime || 0) + (boost.effect?.duration || 0),
          activatedAt: now
        };
      } else {
